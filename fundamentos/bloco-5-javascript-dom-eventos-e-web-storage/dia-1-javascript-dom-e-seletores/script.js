//só para fixar 
/*
document.getElementById("page-title").innerText = "Harry Potter";
document.getElementById("second-paragraph").innerText = "Este é o primeiro filme da saga";
document.getElementById("subtitle").innerText = "E a pedra filosofal";
*/
//só para fixar 2
/*
let par = document.getElementsByClassName("paragrafo");
par[0].style.fontSize = '1.5rem';
par[0].style.color = 'green';
let sub = document.getElementsByTagName('h4');
sub[0].style.color = 'green';
*/
